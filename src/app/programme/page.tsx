// src/app/programme/page.tsx - UPDATED WITH AUTO-REFRESH
...
export default function ProgrammePage() {
  ...
  const loadProgramme = async (): Promise<void> => {
    ...
    try {
      ...
      // 🔥 NOUVEAU : Recalcul global de tous les modules au chargement
      console.log('🔄 Recalcul global des progressions...');
      await ModuleService.recalculateAllModulesProgress(supabaseUserId);
  ...
  /**
   * Actualise les données du programme avec recalcul global
   */
  const handleRefresh = async (): Promise<void> => {
    setState(prev => ({ ...prev, refreshing: true, error: null }));
    
    if (state.userId) {
      console.log('🔄 Refresh avec recalcul global...');
      try {
        // Recalcul global avant rechargement
  ...
  // 🔥 NOUVEAU : Effet pour recalculer automatiquement au retour de page
  useEffect(() => {
    let isActive = true;
    
    const initializeAndRecalculate = async () => {
      if (isActive) {
        await loadProgramme();
      }
    };
    
    initializeAndRecalculate();
  ...
  // 🔥 NOUVEAU : Recalcul automatique périodique (optionnel)
  useEffect(() => {
    if (!state.userId) return;
    
    const interval = setInterval(async () => {
      console.log('🔄 Recalcul automatique périodique...');
      try {
        await ModuleService.recalculateAllModulesProgress(state.userId!);
        // Recharger silencieusement les données
        const programme = await programmeSupabaseService.getProgramme(state.userId!);
        if (programme) {
          setState(prev => ({ ...prev, programmeData: programme }));
        }
      } catch (error) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-pink-100 to-purple-100 flex items-center justify-center">
        <div className="bg-white rounded-2xl shadow-xl p-8 text-center max-w-md">
          ...
          <p className="text-gray-600 mb-4">Connexion à Supabase et recalcul des progressions...</p>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center">
              ...
              <span className="text-green-800 font-medium">
                Connecté à Supabase - Progressions recalculées
              </span>
              {state.userEmail && (
            ...
            >
              ...
              <span className="text-sm">
                {state.refreshing ? 'Recalcul...' : 'Recalculer'}
              </span>
            </button>
          </div>
        </div>*/}
        ...
        <div className="bg-white rounded-3xl shadow-xl p-8 mb-8">
          ...
          <div className="relative mb-6">
            ...
            {/*<p className="text-center text-gray-600 mt-2 font-medium">
              Progression globale du programme (recalculée automatiquement)
            </p>*/}
          </div>
